...
function [H,C,Cg] = ForwardDynamicsCentroidal(q,qD)  
  global robot
  ant = robot.ant;
  frames = robot.frames;
  ...
  [X,J,CJ,p1c] = computeC(robot,piTi,qD,true);
  C = [p1c;CJ];
  ...
  H(1:6,7:end) = F2;
  H(7:end,1:6) = F2';
  [~,~,CJg,p1cg] = computeC(robot,piTi,qD,false);
  Cg = [p1cg;CJg];
end


function [X,J,C,p1c] = computeC(robot,T,qD,gravityOn)
baseDof = 6;
act = robot.act;
ant = robot.ant;
frames = robot.frames;
