function [CoM,J_CoM,J_RAnkle,J_LAnkle,crossM,J_CoMs] = compute_com(robot)
...
Jb = baseJacobian(T(:,:,1),MS_b,Yaw,Pitch);
Jb = M(1)*Jb;
J_CoM(:,1:6) = Jb;
CoM = CoM + M(1)*MS_b;
for j = 2 : 28
    if M(j) ~=0
        ...
        J_X(1:3,1:6) = baseJacobian(T(:,:,1),MS_j,Yaw,Pitch);
...
Jb_RAnkle = baseJacobian(T(:,:,1),T(1:3,4,8),Yaw,Pitch);
J_RAnkle(4:6,1:6) = Jb_RAnkle;
while F~=1
        if act(F)~=0 
            J = crossM(:,:,F)*(T(1:3,4,8)-T(1:3,4,F));
            J_RAnkle(4:6,act(F)+6) = J(1:3);
        end
        F = ant(F);
end
...
Jb_LAnkle = baseJacobian(T(:,:,1),T(1:3,4,15),Yaw,Pitch);
J_LAnkle(1:3,1:6) = Jb_LAnkle;
while F~=1
        if act(F)~=0 
            J = crossM(:,:,F)*(T(1:3,4,15)-T(1:3,4,F));
            J_LAnkle(1:3,act(F)+6-1) =J(1:3);
        end
        F = ant(F);
end
...
function Jb = baseJacobian(T,p,R,P)
    Jb = zeros(6,6);
    Jb(4:6,4:6) = eye(3,3);
    Jb(1:3,4:6) = cross_matrix(T(1:3,4,1)-p)*inv(OmeRPY(R,P)); %OpO1 x OmegaInv
end
